...
if(isset($_POST['submit'])){
    ...
    if (count($rows) > 0) {
      ...
      $insertIntoAdmin = "INSERT INTO admin(nom, prenom, email, password, image) VALUES (:nom, :prenom, :email, :password, :file_name)";
  
      // Prepare SQL statements
      $queryForUsers = $conn->prepare($insertIntoUsers);
      $queryForAdmin = $conn->prepare($insertIntoAdmin);
      ...
      // Bind parameters and execute queries for both tables
      $queryForAdmin->bindParam(':nom', $nom, PDO::PARAM_STR);
      $queryForAdmin->bindParam(':prenom', $prenom, PDO::PARAM_STR);
      $queryForAdmin->bindParam(':email', $email, PDO::PARAM_STR);
      $queryForAdmin->bindParam(':password', $pass, PDO::PARAM_STR);
      $queryForAdmin->bindParam(':file_name', $file_name, PDO::PARAM_STR);
      $queryForAdmin->execute();
      ...
      echo "<div class='alert alert-success'>L'admin a été ajoutee avec succès!</div>";



  
      exit(); // Ensure script stops executing after insertions
  }
